...
Generated on: 2026-03-07 00:44:01

| Status | URI | Rule ID | Message | Matched Data |
| :--- | :--- | :--- | :--- | :--- |
| 403 | `/etc/passwd` | **990001** | Custom LFI Protection: etc/passwd access denied | `-` |
...
| 403 | `/ftp/%252e%252e%252f%252e%252e%252fetc/passwd` | **949110** | Inbound Anomaly Score Exceeded (Total Score: 5) | `-` |
...
| 403 | `/rest/user/reset-password?email=%7B%7B7*7%7D%7D` | **990011** | Custom SSTI Protection: Template markers detected | `-` |
...
| 403 | `/b2b/v2/orders` | **941100** | XSS Attack Detected via libinjection | `XSS data found within ARGS:json.cid: <script>alert(1)</script>` |
| 403 | `/b2b/v2/orders` | **941110** | XSS Filter - Category 1: Script Tag Vector | `<script> found within ARGS:json.cid: <script>alert(1)</script>` |
| 403 | `/b2b/v2/orders` | **941160** | NoScript XSS InjectionChecker: HTML Injection | `<script found within ARGS:json.cid: <script>alert(1)</script>` |
| 403 | `/api/Feedbacks` | **941170** | NoScript XSS InjectionChecker: Attribute Injection | ``javascript:alert( found within ARGS:json.comment: <iframe src=`javascript:alert(1)`>` |
| 403 | `/api/Feedbacks` | **941210** | IE XSS Filters - Attack Detected | `javascript:a found within ARGS:json.comment: <iframe src=`javascript:alert(1)`>` |
